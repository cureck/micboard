...
def discover():
    ...
    # SO_REUSEPORT is only available on Unix-like systems (Linux, macOS)
    if hasattr(socket, 'SO_REUSEPORT'):
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
    
    # On Windows, we must bind to INADDR_ANY (''), not the multicast address
    # On Unix/Linux, we can bind to the multicast address directly
